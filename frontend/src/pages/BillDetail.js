...
const BillDetail = () => {
  ...
  const handleGeneratePDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(18);
    doc.text(`Bill #${bill?.id || ''}`, 14, 18);
    doc.setFontSize(12);
    doc.text(`Status: ${formData.status || ''}`, 14, 28);
    doc.text(`Date: ${bill?.created_at ? new Date(bill.created_at).toLocaleDateString() : ''}`, 80, 28);
    doc.text(`Customer: ${customers.find(c => c.id === formData.customer)?.first_name || ''} ${customers.find(c => c.id === formData.customer)?.last_name || ''}`, 14, 38);
    doc.text(`Appointment: ${appointments.find(a => a.id === formData.appointment) ? `#${appointments.find(a => a.id === formData.appointment).id} - ${appointments.find(a => a.id === formData.appointment).description}` : ''}`, 14, 46);
    doc.text(`Description: ${formData.description || ''}`, 14, 54);
    doc.text(`Notes: ${formData.notes || ''}`, 14, 62);
    doc.text(`Due Date: ${formData.due_date || ''}`, 14, 70);
    doc.text(`Employee Name: ${formData.employee_name || ''}`, 80, 70);
    ...
    const tableRows = formData.line_items.map(item => [
      item.description,
      item.is_labor ? item.employee_number : item.part_number,
    ...
    doc.text(`Total: $${total.toFixed(2)}`, 14, (doc.lastAutoTable?.finalY || 90) + 10);

    doc.save(`Bill_${bill?.id || 'draft'}.pdf`);
  };
  ...
  return (
    <Box sx={{ width: '100%', px: 3, mt: 4, mb: 4 }}>
      <Button variant="outlined" sx={{ mb: 2 }} onClick={handleGeneratePDF}>
        Generate PDF
      </Button>
      <Box display="flex" alignItems="flex-start" gap={3}>
        <Box sx={{ flex: '1 1 0%', minWidth: 0 }}>
          <Card className="bill-print-card">
            <CardContent>
        ...
        <Box sx={{ width: 320, maxWidth: 340, flexShrink: 0 }}>
          <Card>
            <CardContent>
